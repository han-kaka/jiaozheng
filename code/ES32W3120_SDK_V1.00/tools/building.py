...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    Env = env
    ES_root = os.path.abspath(root_directory)
    sys.path = sys.path + [os.path.join(ES_root, 'tools')]

    # add compability with Keil MDK 4.6 which changes the directory of armcc.exe
    if esconfig.PLATFORM == 'armcc':
        if not os.path.isfile(os.path.join(esconfig.EXEC_PATH, 'armcc.exe')):
            if esconfig.EXEC_PATH.find('bin40') > 0:
                esconfig.EXEC_PATH = esconfig.EXEC_PATH.replace('bin40', 'armcc/bin')
    ...
    if env['PLATFORM'] == 'win32':
        os.environ['PATH'] = esconfig.EXEC_PATH + ";" + os.environ['PATH']
    else:
        os.environ['PATH'] = esconfig.EXEC_PATH + ":" + os.environ['PATH']
    ...
    # parse esconfig.h to get used component
    ...
    if esconfig.PLATFORM == 'gcc':
        contents = ''
        if not os.path.isfile('cconfig.h'):
            import gcc
            gcc.GenerateGCCConfig(esconfig)
    ...
    if tgt_name:
        ...
        SetOption('no_exec', 1)
        try:
            esconfig.CROSS_TOOL, esconfig.PLATFORM = tgt_dict[tgt_name]
        except KeyError:
            print 'Unknow target: %s. Avaible targets: %s' % \
                    (tgt_name, ', '.join(tgt_dict.keys()))
            sys.exit(1)
    elif (GetDepend('RT_USING_NEWLIB') == False and GetDepend('RT_USING_NOLIBC') == False) \
        and esconfig.PLATFORM == 'gcc':
    ...
    AddOption('--genconfig', 
                ...
                action = 'store_true',
                default = False, 
                help = 'Generate .config from esconfig.h')
    ...
    AddOption('--useconfig',
                dest = 'useconfig',
                type='string',
                help = 'make esconfig.h from config file.')
    configfn = GetOption('useconfig')
    if configfn:
        from menuconfig import mk_esconfig
        mk_esconfig(configfn)
        exit(0)
    ...
    objs = SConscript('SConscript', variant_dir='MDK-ARM/Objects', duplicate=0)
...
def PrepareModuleBuilding(env, root_directory, bsp_directory):
    ...
    Env = env
    ES_root = root_directory

    # parse bsp esconfig.h to get used component
    PreProcessor = PatchedPreProcessor()
    f = file(bsp_directory + '/esconfig.h', 'r')
    contents = f.read()
    f.close()
    PreProcessor.process_contents(contents)
    BuildOptions = PreProcessor.cpp_namespace
...
def LocalOptions(config_project_name):
    from SCons.Script import SCons

    # parse wiced_config.h to get used component
    PreProcessor = SCons.cpp.PreProcessor()

    f = file(config_project_name, 'r')
    contents = f.read()
    f.close()
...
def EndBuilding(target, program = None):
    ...
    Env.AddPostAction(target, esconfig.POST_ACTION)
    ...
    if GetOption('target') == 'mdk4':
        ...
        if os.path.exists(SCONSTRUCT_DIR + '/MDK-ARM'):
            pass
        else:
            os.mkdir(SCONSTRUCT_DIR + '/MDK-ARM')

        MDK4Project(os.path.join(SCONSTRUCT_DIR + '/MDK-ARM/' + project_name[1] + '.uvproj'), Projects)
    ...
    if GetOption('target') == 'mdk5':
        ...
        MDK5Project(os.path.join(SCONSTRUCT_DIR + '/MDK-ARM/' + project_name[1] + '.uvprojx'), Projects)

    if GetOption('target') == 'iar':
        from iar import IARProject
        from keil import project_name

        if os.path.exists(SCONSTRUCT_DIR + '/IAR'):
            pass
        else:
            os.mkdir(SCONSTRUCT_DIR + '/IAR')

        IARProject('IAR\\' + os.path.join(project_name[1] + '.ewp'), Projects)
...
def GetVersion():
    ...
    rtdef = os.path.join(ES_root, 'include', 'rtdef.h')
