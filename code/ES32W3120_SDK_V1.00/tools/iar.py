...
def IARWorkspace(target):
    ...
    xml = iar_workspace % target[1]

    out.write(xml)
    out.close()

    if os.path.exists(PACK_ROOT + 'build_tools/template.ewd'):
        import shutil
        shutil.copy2(PACK_ROOT+'/build_tools/template.ewd', SCONSTRUCT_DIR+'/IAR/'+project_name[1]+'.ewd')

def IARProject(target, script):
    project_path = os.path.dirname(os.path.abspath(target))

    tree = etree.parse(PACK_ROOT + 'build_tools/template.ewp')
    root = tree.getroot()
