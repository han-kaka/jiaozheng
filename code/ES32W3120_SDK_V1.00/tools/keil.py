...
PROJ_ROOT = SCONSTRUCT_DIR
PACK_ROOT = ''
countinue = 1
while countinue:        #find the PACK directory
    if PROJ_ROOT[-9:-1] != "Projects":      #PROJ_ROOT is "Projects"
        SCONSTRUCT_DIR += "/.."
        PROJ_ROOT = os.path.normpath(SCONSTRUCT_DIR + "/..") + '\\'
        PACK_ROOT = os.path.normpath(SCONSTRUCT_DIR) + '\\'
    else:
        countinue = 0
        SCONSTRUCT_DIR = os.getcwd()

#define project name.
project_name = os.path.split(SCONSTRUCT_DIR)

def MDK4Project(target, script):
    template_tree = etree.parse(PACK_ROOT + 'build_tools/template.uvproj')
...
def MDK5Project(target, script):
    ...
    template_tree = etree.parse(PACK_ROOT + 'build_tools/template.uvprojx')
    ...
    if os.path.isfile(project_uvopt):
        os.unlink(project_uvopt)
    # copy uvopt file
    if os.path.exists(PACK_ROOT + 'build_tools/template.uvoptx'):
        import shutil
        shutil.copy2(PACK_ROOT+'/build_tools/template.uvoptx', SCONSTRUCT_DIR+'/MDK-ARM/'+project_name[1]+'.uvoptx')
