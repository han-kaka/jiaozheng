...
ald_status_t ald_adc_init(adc_handle_t *hperh)
{
    ...
    hperh->perh->CCR1=0;
    hperh->perh->CCR2=0;
	MODIFY_REG(hperh->perh->CCR1, ADC_CCR1_GAINCALEN_MSK, DISABLE << ADC_CCR1_GAINCALEN_POS);
	MODIFY_REG(hperh->perh->CCR1, ADC_CCR1_OFFCALEN_MSK, DISABLE << ADC_CCR1_OFFCALEN_POS);

	/* if the ADC clock less than 1MHz,PWRMOD should be disable*/
	MODIFY_REG(hperh->perh->CCR2, ADC_CCR2_ADRLP_MSK, DISABLE << ADC_CCR2_ADRLP_POS);
	MODIFY_REG(hperh->perh->CCR1, ADC_CCR1_VRBUFEN_MSK, ENABLE << ADC_CCR1_VRBUFEN_POS);
	MODIFY_REG(hperh->perh->CCR1, ADC_CCR1_VCMBUFEN_MSK, ENABLE << ADC_CCR1_VCMBUFEN_POS);
	MODIFY_REG(hperh->perh->CCR1, ADC_CCR1_VREFEN_MSK, ENABLE << ADC_CCR1_VREFEN_POS);
	MODIFY_REG(hperh->perh->CCR1, ADC_CCR1_IREFEN_MSK, ENABLE << ADC_CCR1_IREFEN_POS);

	MODIFY_REG(hperh->perh->CCR1, ADC_CCR1_VRNSEL_MSK, hperh->init.n_ref << ADC_CCR1_VRNSEL_POS);
	MODIFY_REG(hperh->perh->CCR1, ADC_CCR1_VRPSEL_MSK, hperh->init.p_ref << ADC_CCR1_VRPSEL_POSS);
	MODIFY_REG(hperh->perh->CCR1, ADC_CCR1_DIFFEN_MSK, DISABLE << ADC_CCR1_DIFFEN_POS);
	MODIFY_REG(hperh->perh->CCR1, ADC_CCR1_CKDIV_MSK, hperh->init.div << ADC_CCR1_CKDIV_POSS);
	/* Enable adc calibration */
	SET_BIT(hperh->perh->CCR1, ADC_CCR1_TRMEN_MSK);
	MODIFY_REG(hperh->perh->CON1, ADC_CON1_NCHESEL_MSK, hperh->init.nche_sel << ADC_CON1_NCHESEL_POS);
//    SET_BIT(hperh->perh->CCR1, ADC_CCR1_CHPBF_EN_MSK);
//    SET_BIT(hperh->perh->CCR1, ADC_CCR1_CHNBF_EN_MSK);
...
ald_status_t ald_adc_reset(adc_handle_t *hperh)
{
	...
	WRITE_REG(hperh->perh->CCR1, 0x0);
    WRITE_REG(hperh->perh->CCR2, 0x0);
...
/**
  * @brief  Timer triggers insert channel.
  * @param  config: Structure of timer trigger ADC.
  * @retval Status, see @ref ald_status_t.
  */
ald_status_t ald_adc_timer_trigger_insert(adc_timer_config_t *config)
{
    ...
    else if (config->p_timer == AD16C4T0)
    ...
    if (config->p_adc == ADC)
        config->h_pis.init.consumer_trig = PIS_CH7_ADC_ICHTRG;
    ...
    config->h_adc.perh->CON1 |= 0x00100000;
...
ald_status_t ald_adc_timer_trigger_adc_by_dma(adc_timer_config_t *config)
{
	...
	if (config->p_adc == ADC)
    {
        config->h_pis.init.consumer_trig = PIS_CH6_ADC_NCHTRG;
    }
